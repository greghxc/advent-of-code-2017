...
public class Program {
    Queue<Long> publishQueue;
    Queue<Long> listenQueue;
    ...
    Program(Integer programId, Queue publishQueue, Queue listenQueue, File file) {
        ...
        registers.put("p", Long.valueOf(programId));
    }
    ...
    void rcv(String x) throws NoMessageException {
        if (listenQueue.hasNext()) {
            registers.put(x, listenQueue.getMsg());
        } else {
            throw new NoMessageException("Message not ready.");
        }
        currentInstruction++;
    }
